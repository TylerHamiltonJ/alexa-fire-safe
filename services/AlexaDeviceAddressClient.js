...
class AlexaDeviceAddressClient {
  ...
  getFullAddress() {
    const options = this.__getRequestOptions(
      `/v1/devices/${this.deviceId}/settings/address`
    );
  ...
  __handleDeviceAddressApiRequest(requestOptions, fulfill, reject) {
    ...
    Https.get(requestOptions, response => {
      console.log(
        `Device Address API responded with a status code of : ${response.statusCode}`
      );
      ...
      response.on('data', data => {
        ...
        console.log('Parsing payload', responsePayloadObject);
